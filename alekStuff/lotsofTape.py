    ...
    if r:
        ...
        if len(points) > 1:
            ...
            if len(goodPoints) >= 0:
                ...
                # sort by distance
                goodPoints = goodPoints.sort(key = abs(points[curTuple[2]].pt[0] - points[curTuple[1]].pt[0])) # tbh dont really need abs, it is certainly positive
                ...
                for i in range(len(top3)): # note: len(top3) need not be 3, if we didn't find 3
                    ...
                    centerScores.append( abs((points[top3[i][2]].pt[0] + points[top3[i][1]].pt[0]) * 0.5 - mask.shape[1] * 0.5), points[top3[i]] )
                centerScores.sort(key=0)
                ...
                dists = [abs(points[centerScores[i][1][2]].pt[0]-points[top3[i][1][1]].pt[0]) for i in range(len(top3))]
                if dists[0] > dists[1]*1.5:
                ...
                cv2.circle(bgr, (points[goal_i][0], points[goal_i][1]), 10, (0,0,0), 5)
                cv2.circle(bgr, (points[goal_j][0], points[goal_j][1]), 10, (0,0,0), 5)
